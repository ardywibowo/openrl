...
def unique_run_name_from_configs(config_filenames):
    ...
    unique_name = f"{configs}_{current_time}"
...
def include_paths(base_dir, rel_path, include_dir):
...
def load_config_object(filenames: List[str]) -> Dict[str, Any]:
    import _jsonnet
    import json
    
    CONFIG_PATH = os.path.abspath(os.environ.get("CONFIG_PATH", "configs"))
    ...
    json_str = _jsonnet.evaluate_snippet(
        ...
        jsonnet_str, 
        ext_vars=ext_vars, 
        import_callback=lambda base_dir, rel_path: include_paths(base_dir, rel_path, CONFIG_PATH)
    )
...
def load_jsonnet_config(config_paths: List[str]) -> Dict[str, Any]:
    import _jsonnet
    import json
