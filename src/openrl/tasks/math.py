...
from openrl.tasks.math_answer_extraction import (
    extract_math_answer, extract_math_minerva_few_shot_cot_answer)
from openrl.tasks.math_grader import grade_answer
from openrl.tasks.math_grader_minerva import eval_math
...
class MATH(Task):
    ...
    def build_dataset(self) -> DatasetDict:
        ...
        if self.use_minerva_few_shot_prompt:
            map_fn = self._get_preprocess_fn_for_minerva_prompt()
        else:
    ...
    def _get_preprocess_fn_for_minerva_prompt(self):
        tokenizer = self.tokenizer
        prepend_in_context_few_shot = self.prepend_in_context_few_shot
        ensure_fit_in_context_size = self.ensure_fit_in_context_size
        context_size = self.context_size
    ...
    def _split_solution_into_intermediate_steps_minerva(
        ...
        from openrl.tasks.math_extract_steps_inplace import \
