...
from openrl.common import JsonDict
from openrl.common.logging_utils import get_logger
from openrl.common.py_utils import is_flash_attention_model
...
class PolicyTrainer(Trainer):
    ...
    def _create_optimizer(self) -> Union[torch.optim.Optimizer, DummyOptim]:
        ...
        embedding_parameters = [
            n for n, p in opt_model.named_parameters() if "embedding" in n.lower()
        ]
        decay_parameters = [
            name
            for name in decay_parameters
            if "bias" not in name and name not in embedding_parameters
        ]
