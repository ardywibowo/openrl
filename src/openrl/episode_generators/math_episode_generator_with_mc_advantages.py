...
from openrl.episode_generators import EpisodeGenerator, MathEpisodeGenerator
...
class MathEpisodeGeneratorWithMCAdvantages(MathEpisodeGenerator):
    ...
    def _update_trajectories_w_values(
        ...
        for res in tqdm(
            ...
            infer_tree = json.loads(res["_openrl__reasoning_tree"])
    ...
    def _create_trajectories(
        ...
        for idx, instance in enumerate(inference_results):
            ...
            for path in paths:
                ...
                # noinspection PyUnresolvedReferences
                data_instance = {
                    k: v for k, v in instance.items() if not k.startswith("_openrl")
                }
